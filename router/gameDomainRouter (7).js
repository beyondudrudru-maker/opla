...
// ─────────────────────────────────────────────────────────────────────────────
// Complements (does not replace) the existing exact word-boundary match and
// the no-space substring "typo" match above. Those two catch missing spaces
// or exact-but-glued names; they do NOT catch genuine misspellings like
// "Dagno" for "Dagon" or "Bonebraker" for "Bonebreaker" — a single
// transposed/dropped/substituted letter never produces a no-space substring
// hit. This adds that missing case via classic Levenshtein edit-distance,
// checked per-word against each known entity name (or against each word of
// a multi-word name), so a single typo'd token in a longer sentence still
// resolves correctly.
//
...
// ─────────────────────────────────────────────────────────────────────────────
// 🔤 FUZZY / TYPO MATCHING — Damerau-Levenshtein distance
// ─────────────────────────────────────────────────────────────────────────────
// Complements (does not replace) the existing exact word-boundary match and
// the no-space substring "typo" match above. Those two catch missing spaces
// or exact-but-glued names; they do NOT catch genuine misspellings like
// "Dagno" for "Dagon" or "Bonebraker" for "Bonebreaker" — a single
// transposed/dropped/substituted letter never produces a no-space substring
// hit. This adds that missing case via Damerau-Levenshtein edit-distance
// (which, unlike plain Levenshtein, prices an adjacent-letter transposition
// as ONE edit instead of two — "dagon"->"dagno" is a transposition), checked
// per-word against each known entity name (or against each word of a
// multi-word name), so a single typo'd token in a longer sentence still
// resolves correctly.
//
// Same Scunthorpe-safe gate as the existing typo pass: only names longer
// than 4 characters are eligible, so short names ("Imp", "Ash", "Kai")
// never get a fuzzy pass and can't misfire on unrelated short words.
//
// 🐛 FIX: plain Levenshtein was used originally, which charges 2 (not 1) for
// a transposition — so _maxEditDistanceFor had to allow distance-2 for any
// name >= 5 chars just to catch "dagon"/"dagno". That same distance-2 window
// was wide enough to also match totally unrelated real-world words purely by
// coincidence: "bengal" (as in "West Bengal") is plain-Levenshtein distance 2
// from the hero name "Benua" (Baron Benua), so a message about a real-world
// flood got misrouted into the game lane. Damerau-Levenshtein correctly
// scores the genuine transposition case ("dagon"/"dagno") as distance 1, so
// the threshold below no longer needs the extra slack that was causing the
// false positive.
function _damerauLevenshtein(a, b) {
  ...
  for (let i = 1; i <= al; i++) {
    for (let j = 1; j <= bl; j++) {
      ...
      // Transposition: swap of two adjacent characters costs 1, not 2.
...
// Max edit distance allowed, scaled to name length. Now that transpositions
// are priced correctly at 1 (not 2), a tight distance-1 threshold up through
// 6-char words still catches real typos ("dagon"/"dagno" = 1,
// "gollem"/"golem" = 1, "bonebreaker"/"bonebraker" = 1) without the
// distance-2 window that let unrelated same-length words collide
// ("bengal"/"benua" = 2, correctly rejected now).
function _maxEditDistanceFor(len) {
  if (len <= 6) return 1;  // single substitution/insertion/deletion OR one transposition
  if (len <= 9) return 2;  // longer names can tolerate two edits before ambiguity risk rises
  return 3;                // long multi-syllable names (e.g. "Firetamer Anavin" tokens)
}

/**
 * _fuzzyNameHit(candidateName, textWords)
 ...
 */
function _fuzzyNameHit(candidateName, textWords) {
  const nameWords = candidateName.split(/\s+/).filter(w => w.length > 4);
  if (nameWords.length === 0) return false;

  for (const nameWord of nameWords) {
    const maxDist = _maxEditDistanceFor(nameWord.length);
    for (const textWord of textWords) {
      if (textWord.length <= 4) continue; // same short-name safety gate
      // Cheap pre-filter before running full edit-distance: skip pairs whose
      ...
      if (_damerauLevenshtein(nameWord, textWord) <= maxDist) return true;
    }
  }
  return false;
}
...
function route(text, recentContext = '', userCorrections = []) {
  ...
  const _bossTextWords = normalizedText.split(/\s+/).filter(Boolean);
  const matchedBossKeywords = BOSS_KEYWORDS.filter(k => {
    ...
    const fuzzyHit        = !wordBoundaryHit && !noSpaceHit && _fuzzyNameHit(k, _bossTextWords);
  ...
  allKnownHeroes.forEach(h => {
    ...
    const fuzzyMatch = !exactMatch && !typoMatch && _fuzzyNameHit(hName, textWordsForFuzzy);
  ...
  allKnownTroops.forEach(t => {
    ...
    const fuzzyMatch = !exactMatch && !typoMatch && _fuzzyNameHit(tName, textWordsForFuzzy);
  ...
  // 🆕 REWRITE: previous versions matched synergy purely by shared faction/
  // type/tags/categories, which is exactly how "Bonebreaker" ended up paired
  // with "Bone Dragon" — they share nothing but a substring/tag collision,
  // not an actual designed synergy. This now calls queryEngine.getSynergyContext(),
  // which reads ONLY the explicit recommendedHeroes / recommendedTroops /
  // optimalGear references stored directly on the entity's own record.
  // No tag/faction/substring inference of any kind happens here anymore.
  //
